...
def get_problem_responses(request, course_id):
    ...
    Responds with BadRequest if problem URL is faulty.
    """
    course_key = CourseKey.from_string(course_id)
    problem_url = request.GET.get('problem_url', '')

    try:
        problem_key = UsageKey.from_string(problem_url)
        # Are we dealing with an "old-style" problem URL?
        run = getattr(problem_key, 'run')
        if not run:
            problem_key = course_key.make_usage_key_from_deprecated_string(problem_url)
        if problem_key.course_key != course_key:
            raise InvalidKeyError(type(problem_key), problem_key)
    except InvalidKeyError:
        return JsonResponseBadRequest(_("Could not find problem with this URL."))

    try:
        instructor_task.api.submit_calculate_problem_responses_csv(request, course_key, problem_url)
        success_status = _(
