...
class TestInstructorAPILevelsDataDump(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_get_problem_responses_invalid_url(self):
        """
        Test whether get_problem_responses returns an appropriate status
        message when users submit an invalid problem URL.
        """
        url = reverse(
            'get_problem_responses',
        ...
        response = self.client.get(url, {'problem_url': problem_url})
        res_json = json.loads(response.content)
        self.assertEqual(res_json, 'Could not find problem with this URL.')

    def valid_problem_url(test):  # pylint: disable=no-self-argument
        """
        Decorator for tests that target get_problem_responses endpoint and
        need to pretend user submitted a valid problem URL.
        """
    ...
    def test_calculate_report_csv_success(self, report_type, instructor_api_endpoint, task_api_endpoint, extra_instructor_api_kwargs):
        ...
        if report_type == 'problem responses':
            with patch(task_api_endpoint):
                response = self.client.get(url, {'problem_url': ''})
